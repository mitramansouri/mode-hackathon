...
var MessageManager = class {
  ...
  async _shouldRespondBasedOnContext(message2, chatState) {
    ...
    const lastSelfMemories = await this.runtime.messageManager.getMemories({
      roomId: stringToUuid(message2.chat.id.toString() + "-" + this.runtime.agentId),
      unique: false,
      count: 5
    });
  ...
  async isGroupAuthorized(ctx) {
    ...
    if (!allowedGroups.includes(currentGroupId)) {
      ...
      try {
        ...
        elizaLogger2.error(`Error leaving unauthorized group ${currentGroupId}:`, error);
      }
      return false;
    }
    return true;
      ...
      try {
        ...
        if (error?.response?.error_code !== 403) {
          try {
            ...
            elizaLogger2.error("Failed to send error message:", replyError);
          }
        }
      }
    });
...
// src/actions/send_poll.ts
var send_poll_default = {
  name: "CREATE_POLL",
  ...
  validate: async (runtime, message2, state) => {
    if (!message2.content || message2.content.source !== "telegram") {
      return false;
    }
    if (!message2.content.text || !/^\/(?:create)?poll\b/i.test(message2.content.text)) {
      return false;
    }
    return true;
  },
  handler: async (runtime, message2, state, options, callback) => {
    try {
      if (!state) {
        state = await runtime.composeState(message2);
      }
      const pollDetails = message2.content.text.split("\n");
      ...
      const options2 = pollDetails.slice(2).map((option) => option.trim());
      if (!question || options2.length < 2 || options2.length > 10) {
        throw new Error(
      ...
      const response = {
        ...
        action: "SEND_POLL",
        metadata: {
          chat_id: message2.content.chat_id,
          question,
          options: options2,
          is_anonymous: true,
          // You can make this configurable
          type: "regular"
          // or 'quiz' if you want to support quiz polls
        }
      };
...
var TelegramClientInterface = {
  start: async (runtime) => {
    ...
    runtime.registerAction(send_poll_default);
    await tg.start();
